...
export function SettingsPage() {
  ...
  async function handleDeleteRemoteFarm() {
    ...
    if (!window.confirm(\Tem certeza que deseja EXCLUIR a fazenda online "\"? Esta ação não pode ser desfeita.\)) {
      return;
    }
  ...
  async function handleRemoveMember(member: RemoteFarmMember) {
    if (!window.confirm(\Remover o membro \?\)) {
      return;
    }
            ...
            <form onSubmit={handleSubmit} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-slate-500">
                  {settings ? \Atualizado em \\ : 'Ainda sem configuração salva.'}
                </p>
                <button
                  type="submit"
