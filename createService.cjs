...
export function simulateCross(femaleId: string, maleId: string, animals: Animal[], semenList: Semen[]): CrossSimulationResult {
  ...
  if (!isCritical) {
    const femaleParents = [female.father_id, female.mother_id].filter(Boolean);
    const maleParents = maleAnimal ? [maleAnimal.father_id, maleAnimal.mother_id].filter(Boolean) : [];
  ...
  if (isCritical) { inbreedingLevel = 'critical'; inbreedingMessage = 'ALERTA CRITICO: Parentesco de 1o Grau (Risco severo de defeitos e perda de fertilidade).'; score -= 8; }
  else if (isHighAlert) { inbreedingLevel = 'high'; inbreedingMessage = 'ALERTA ALTO: Parentesco de 2o Grau (Consanguinidade de ~12.5%).'; score -= 4; }
  ...
  return { score: Math.max(0, score), inbreedingLevel: inbreedingLevel as any, inbreedingMessage, heterosisLevel: heterosisLevel as any, heterosisMessage };
}
\;
