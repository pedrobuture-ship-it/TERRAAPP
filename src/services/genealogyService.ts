...
export function buildFamilyTree(animalId: string, animals: Animal[], semenList: Semen[]): FamilyTree | null {
  ...
  if (father && father.type === 'animal') {
    ...
    if (fA) {
      paternalGrandfather = resolveParent(fA.father_id, animals, semenList);
      paternalGrandmother = resolveParent(fA.mother_id, animals, semenList);
    }
  }
  if (mother && mother.type === 'animal') {
    const mA = animals.find((a) => a.id === mother.id);
    if (mA) {
      maternalGrandfather = resolveParent(mA.father_id, animals, semenList);
      maternalGrandmother = resolveParent(mA.mother_id, animals, semenList);
    }
  }
...
function getAncestors(animalId: string, animals: Animal[], depth: number, currentDepth = 0): string[] {
  ...
  if (animal.father_id) {
    ancestors.push(animal.father_id);
    ancestors.push(...getAncestors(animal.father_id, animals, depth, currentDepth + 1));
  }
  if (animal.mother_id) {
    ancestors.push(animal.mother_id);
    ancestors.push(...getAncestors(animal.mother_id, animals, depth, currentDepth + 1));
  }
...
export function simulateCross(femaleId: string, maleId: string, animals: Animal[], semenList: Semen[]): CrossSimulationResult {
  ...
  if (female.father_id === maleId) {
    isCritical = true;
  } else if (maleAnimal && maleAnimal.mother_id === femaleId) {
    isCritical = true;
  } else if (
    female.father_id && 
    female.mother_id &&
    maleAnimal &&
    female.father_id === maleAnimal.father_id &&
    female.mother_id === maleAnimal.mother_id
  ) {
    isCritical = true;
  }
  ...
  if (!isCritical) {
    ...
    if (femaleParents.some((p) => maleParents.includes(p))) {
      isHighAlert = true;
    }
    if (femaleParents.some(p => {
        const parent = animals.find(a => a.id === p);
        return parent && parent.father_id === maleId;
    })) {
        isHighAlert = true;
    }
  }
  ...
  if (!isCritical && !isHighAlert) {
    ...
    if (femaleAncestors.some((a) => maleAncestors.includes(a))) {
      isDistant = true;
    } else if (femaleAncestors.includes(maleId) || maleAncestors.includes(femaleId)) {
        isDistant = true;
    }
  }
